...
function init() {
  ...
  $("body").append(
    '<div id="hud"><p>Health: <span id="health">100</span><br />Score: <span id="score">0</span></p></div>'
  );
  $("body").append(
    '<div id="credits"><p>Created by <a href="http://www.mnetlify.netlify.com/">Vedansh Mishra</a> using <a href="http://mrdoob.github.com/three.js/">Three.js</a><br />WASD to move, mouse to look, click to shoot</p></div>'
  );
...
function render() {
  ...
  if (Date.now() > lastHealthPickup + 60000) {
    if (
      distance(
        cam.position.x,
        cam.position.z,
        healthcube.position.x,
        healthcube.position.z
      ) < 15 &&
      health != 100
    ) {
  ...
  for (var i = bullets.length - 1; i >= 0; i--) {
    ...
    // Bullet hits player
    if (
      distance(p.x, p.z, cam.position.x, cam.position.z) < 25 &&
      b.owner != cam
    ) {
      $("#hurt").fadeIn(75);
      health -= 10;
      ...
      val =
        health < 25
          ? '<span style="color: darkRed">' + health + "</span>"
          : health;
      $("#health").html(val);
      bullets.splice(i, 1);
      scene.remove(b);
...
function setupScene() {
  ...
  var floor = new t.Mesh(
    ...
    new t.MeshLambertMaterial({
      color: 0xedcba0 /*map: t.ImageUtils.loadTexture('images/floor-1.jpg')*/
    })
  );
  scene.add(floor);
  ...
  var materials = [
    new t.MeshLambertMaterial({
      /*color: 0x00CCAA,*/ map: t.ImageUtils.loadTexture("images/wall-1.jpg")
    }),
    new t.MeshLambertMaterial({
      /*color: 0xC5EDA0,*/ map: t.ImageUtils.loadTexture("images/wall-2.jpg")
    }),
  ...
  healthcube = new t.Mesh(
    ...
    new t.MeshBasicMaterial({
      map: t.ImageUtils.loadTexture("images/health.png")
    })
  );
...
function addAI() {
  ...
  var aiMaterial = new t.MeshBasicMaterial({
    /*color: 0xEE3333,*/ map: t.ImageUtils.loadTexture("images/face.png")
  });
...
function drawRadar() {
  var c = getMapSector(cam.position),
    context = document.getElementById("radar").getContext("2d");
